...
const createRun = ({
  ...
  if (platformConfig === undefined || _cliConfigApple().supportedPlatforms[platformName] === undefined) {
  ...
  if (platformConfig.automaticPodsInstallation || args.forcePods || args.onlyPods) {
    ...
    await (0, _cliConfigApple().resolvePods)(ctx.root, platformConfig.sourceDir, ctx.dependencies, platformName, ctx.reactNativePath, {
  ...
  let devices = await (0, _listDevices.default)(sdkNames);
  if (devices.length === 0) {
  ...
  if (preferredDevice) {
    const preferredDeviceIndex = devices.findIndex(({
      udid
    }) => udid === preferredDevice);
    if (preferredDeviceIndex > -1) {
      const [device] = devices.splice(preferredDeviceIndex, 1);
      devices.unshift(device);
    }
  }
  const fallbackSimulator = platformName === 'ios' || platformName === 'tvos' ? (0, _getFallbackSimulator.getFallbackSimulator)(args) : devices[0];
  if (args.listDevices || args.interactive) {
    ...
    const selectedDevice = await (0, _prompts.promptForDeviceSelection)(devices);
    if (!selectedDevice) {
  ...
  if (args.udid) {
    ...
    if (((_device = device) === null || _device === void 0 ? void 0 : _device.type) === 'simulator') {
      return (0, _runOnSimulator.runOnSimulator)(xcodeProject, platformName, mode, scheme, args, device);
    } else {
      if (!device) {
    ...
    const matchedSimulator = devices.find(d => d.type === 'simulator' && (d.udid === args.simulator || d.name === args.simulator || (0, _matchingDevice.formattedDeviceName)(d) === args.simulator));
    if (!matchedSimulator) {
      _cliTools().logger.warn(`Could not find a simulator with name or UDID: "${_picocolors().default.bold(args.simulator)}". ${(0, _matchingDevice.printFoundDevices)(devices, 'simulator')}`);
      _cliTools().logger.info('Falling back to default simulator...');
    }
    return (0, _runOnSimulator.runOnSimulator)(xcodeProject, platformName, mode, scheme, args, matchedSimulator ?? fallbackSimulator);
  } else {
