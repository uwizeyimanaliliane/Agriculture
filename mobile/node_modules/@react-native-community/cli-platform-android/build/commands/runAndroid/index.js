...
async function runAndroid(_argv, config, args) {
  ...
  let androidProject = (0, _cliConfigAndroid().getAndroidProject)(config);
...
async function buildAndRun(args, androidProject) {
  ...
  if (args.listDevices || args.interactive) {
    ...
    throw new (_cliTools().CLIError)(`Failed to launch emulator. Reason: ${_picocolors().default.dim(result.error || '')}`);
  }
  if (args.device) {
